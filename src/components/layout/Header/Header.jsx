...
export default function Header() {
  ...
  const searchIndex = useMemo(() => {
    ...
    // 1) Intenciones primero (prioridad alta)
    ...
    // 2) Mega menú
    Object.entries(MEGA_MENU_DATA).forEach(([categoryName, cat]) => {
  ...
  // ✅ Expande búsqueda con sinónimos
  ...
  const findBestHref = (qRaw) => {
    ...
    // Score: 0 es perfecto. Priorizamos priority alto y score bajo
    let best = null;
    ...
    // Evita redirecciones locas si no hay match real
    if (!best) return null;
